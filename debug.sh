...
fifo="$(mktemp -u)"
mkfifo "$fifo"
stdbuf -i0 -o0 -e0 qemu-system-i386 -s -S -machine pc -cpu qemu32 -accel tcg -m 1024 -nodefaults -no-reboot -nographic -serial stdio "$@" < /dev/null 2>&1 > "$fifo" &
qemu_pid="$!"
...
gdb-multiarch -x real-mode.gdbinit "$elf_file"
